...
import { GID, Id, IdMap, IdMap2 } from "~/jumpchain/Types";
...
export default function Index() {
    ...
    const [activeSupplements, setActiveSupplements] = useState<Id<GID.Supplement>[]>(
        searchParams.get("supp") ? [Number(searchParams.get("supp"))] : []
    );
    ...
    const [[searchQuery, searchResults], setSearchData] = useState<[QueryParams, { [key: number]: Id<GID.Purchase>[] }[] | undefined]>([{
        characterId: charId,
        pools: [{ type: PurchaseType.Perk, categories: [-1, ...Object.keys(chain.purchaseCategories[PurchaseType.Perk]).map(Number)] },
        ...activeSupplements.map((suppId) => {
            return {
                categories: supplementCategories[suppId].length > 0 ? supplementCategories[suppId] : [-1, ...Object.keys(chain.supplements[suppId].purchaseCategories).map(Number)],
    ...
    let categories = Object.entries(chain.purchaseCategories[PurchaseType.Perk]);
    ...
    let categorySelect = <Multiselect
        ...
        onChange={(data) => {
            ...
            doSearch({
                ...
                pools: [{
                    categories: cats.length > 0 ? cats : [...Object.keys(chain.purchaseCategories[PurchaseType.Perk]).map(Number), -1],
        ...
        return <Multiselect
            ...
            onChange={(data) => {
                ...
                doSearch({
                    term: searchQuery.term,
                    pools: [{
                        categories: primaryCategories.length > 0 ? primaryCategories : [-1, ...Object.keys(chain.purchaseCategories[PurchaseType.Perk]).map(Number)],
                        type: PurchaseType.Perk
                    },
    ...
    let noResultsFound = <div className="center-text-align roomy-cell">No Perks Matching Query!</div>
